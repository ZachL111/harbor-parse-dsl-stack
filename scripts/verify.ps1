...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-harbor-parse-dsl-detail.ps1

ruby -Ilib tests/test_domain_review.rb

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-harbor-parse-dsl-properties.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-harbor-parse-dsl-golden.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-harbor_parse_dsl_stack-casebook.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
